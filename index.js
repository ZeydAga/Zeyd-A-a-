...
const owner = ["392547694364721162","962466512894066738","999484566068809851","","",]; // اي دي الاونر اللي يقدر يتحكم في اوامر الاونر
...
client.user.setActivity('System Server', { type: 'WATCHING' }) // حط الحاله اللي انت عايز
...
My Name \`${client.user.tag}\`
My Developer <@392547694364721162> 
...
client.on("messageCreate", message => {
    ...
    {
      name:"`المبلغ معا الضرائب:`", value:`**${probot.taxs(args)}**`
...
client.on("messageCreate", message => {
  if(message.content.startsWith(prefix + "fedback")){
    ...
    if(!args) return message.channel.send({ content: "اكتب اقتراحك" })
    var fedbackchannel = message.guild.channels.cache.find(channel => channel.name === "ャ⌠🗯⌡suggestions"); // اسم الروم
    const embed = new Discord.MessageEmbed()
    .setTitle("New suggestions :heart_eyes:")
    .setThumbnail(`${message.author.avatarURL({dynamic : true})}`)
    .setDescription(`${args}`)
    .setFooter({ text: message.author.tag , iconURL: 
     message.author.displayAvatarURL({dynamic:true})})
    message.channel.send({ content: "شكرا لقتراحك" })
...
🆔丨\`ID Discord : 392547694364721162\`

💫丨\`AEG : 16\`

👑丨\`Profile By\` <@392547694364721162>

[Soivel server](https://discord.gg/S392RnpAxd)
[House](https://discord.gg/cr6CcNHWh2)
        `)
      .setFooter({ text: message.author.tag , iconURL: 
     message.author.displayAvatarURL({dynamic:true})})
      .setTimestamp()
